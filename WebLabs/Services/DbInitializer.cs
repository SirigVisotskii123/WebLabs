...
namespace WebLabs.Services
{
    public class DbInitializer
    {
        public static async Task SetupDb(WebApplication app)
        {
			...
			//var Dishes = (DbSet<Dish>?)scope.ServiceProvider.GetService(typeof(DbSet<Dish>));
			//var DishGroups = (DbSet<DishGroup>?)scope.ServiceProvider.GetService(typeof(DbSet<DishGroup>));
        ...
        public static async Task Seed(ApplicationDbContext context, UserManager<ApplicationUser> userManager, RoleManager<IdentityRole> roleManager)
        {
            ...
            if (!context.DishGroup.Any())
            {
                context.DishGroup.AddRange(
                new List<DishGroup>
                {
